...
$.fn.tpl=function(){
  ...
  el.render=function(data,fn){
  	 for(var i=0,l=el.length;i<l;i++){
  	 	  fn(i,el[i].render(data));
  	 }
  };
  this.data(en,el);
  return el;           
...
$.fn.grid=function(opt){
	...
	if(!opt.body){
		 alert("no body defined");
	   return;	
	}
	var en="_el",el=this.data(en),_body,_loaded;
	if(el){
		 return el;
	}
	$.ajaxSetup({
     error:function(x, e) {
       new $.zui.Messager('提示消息：系统错误',{type: 'warning',time:opt.timeout||3000}).show();
       _loaded=false;
       return false;
     }
  });
  var that=this;
	el={
		getBody:function(){
			return _body||(_body=$(opt.body));
		},
		loadData:function(param,fn,error){
			if(!opt.url||_loaded){
	       return;	
			}
			this.param=param||{};
			_loaded=true;
			var it=this;
			$.getJSON(opt.url,this.param,function(json){
				 var msg;
			   if(json){
			   	 if(json.success){
			   	   it.render(json,fn);
			   	   msg=new $.zui.Messager('提示消息：成功',{type:'success',time:opt.timeout||3000}).show();
			   	 }else{
			   	 	 msg=new $.zui.Messager('提示消息：'+json.retMsg,{type:'warning',time:opt.timeout||3000}).show();
			   	 	 error&&error();
			   	 }
			   }else{
			   	  msg=new $.zui.Messager('提示消息：系统错误',{type: 'warning',time:opt.timeout||3000}).show();
			   	  error&&error();
			   }
			   _loaded=false;
			});
		...
		render:function(data,fn){
			if(data){
				...
				var tpl=$(opt.tpl).tpl(),htmls=[],addHtml=function(index,html){
			  	htmls.push(html);
			  };
				for(var i=0,l=data.length;i<l;i++){
					tpl.render(data[i],addHtml);
				}
				this.getBody().html(htmls.join(""));
		  }
		  fn&&fn.call(this,data);
		}
	};
	var parent=$(this).parent();
	parent.on("click",".previous,.next",function(){
			var it=$(this),pageno=it.is(".previous")?-1:1;
			if(pageno){
				el.param=el.param||{};
				if(el.param.start){
					el.param.start=el.param.start+(parseInt(pageno)||0);
				}else{
					el.param.start=1+parseInt(pageno)||0;
				}
				if(el.param.start==1){
					 parent.find(".previous").addClass("disabled");
				}else{
					 parent.find(".previous").removeClass("disabled");
				}
				el.loadData(el.param,function(data){
					if(data.length==0){
...
$.jsGrid.defaultOption={
        ...
        autoload: true,
  	    onDataLoaded:function(grid,data){
        	 grid.grid._body.find(".td-break").each(function(){
			    	 var it=$(this);
			    	 it.popover({
			    	 	 container:"body",
			    	 	 trigger:"hover",
